...
def classifyVid(vidPath, detector, detector2, predictor, SHOW_FRAME = True):
	...
	while NOT_GRABBED < 20:
		...
		# detect faces in the grayscale frame
		rect = detectFaces(gray,detector,detector2)
...
def classifyPic(picPath, detector, detector2, predictor):

	# Load image in greyscale
	img = cv2.imread(picPath,0)
	rect = detectFaces(gray,detector)
...
for filename in os.listdir(vidPath):
	...
	if(filename.endswith(".MP4") or filename.endswith(".mp4")):
		...
		classifyVid(vidPath + filename,detector,detector2,predictor,SHOW_FRAME = True)
